...
// that the planets produce harmonies based on their intervals.
// Kepler made it rigorous in Harmonices Mundi (1619).
//
// We score frequency assignments by how close each planet pair's
// frequency ratio is to a pure Pythagorean interval.
// ═══════════════════════════════════════════════════════════════
...
///
/// At 0.015 only genuinely close matches score 0. Equal-tempered semitones
/// deviate ~1.1% from pure Pythagorean ratios — they still pass, but
/// quarter-tones and trap frequencies do not. This is tight enough that
/// the solver must be selective about which pitches form consonant pairs.
const TOLERANCE: f64 = 0.015;

/// Compute the harmonic deviation score (penalty) and nearest interval
/// for two frequencies f_a and f_b.
///
...
pub fn harmonic_deviation(f_a: f64, f_b: f64) -> (i64, &'static Interval, f64) {
    if f_a <= 0.0 || f_b <= 0.0 {
        return (1000, &INTERVALS[0], 1.0);
    }

    // Normalise: always larger/smaller >= 1.0
    let ratio = if f_a >= f_b { f_a / f_b } else { f_b / f_a };
    ...
    // Score: 0 when deviation is within TOLERANCE of any interval.
    // Flat threshold — no consonance scaling. Equal-tempered semitones
    // deviate ~0.1-1.1% from pure ratios, all well within 1.5%.
    // Quarter-tones and traps deviate 2-4%, so they fail.
    if best_dev <= TOLERANCE {
        // Close enough — no penalty
        return (0, best_interval, best_dev);
    }
    // Beyond threshold: penalty scaled by (1 - consonance) so landing near
    // a dissonant interval hurts more than landing near a consonant one.
    let excess = (best_dev - TOLERANCE) / TOLERANCE;
    let base_penalty = excess.min(1.0) * 1000.0;
    // Dissonance multiplier: consonance=1.0 → 1x, consonance=0.1 → 1.9x
    let dissonance_mult = 2.0 - best_interval.consonance;
    let penalty = (base_penalty * dissonance_mult) as i64;
